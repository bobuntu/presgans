...
def _helper(Gen, x_tilde, eps, sigma):
...
def get_samples(Gen, x_tilde, eps_init, sigma, burn_in, num_samples_posterior,
  ...
  for i in range(n_steps):
    ...
    current_p = p
    logjoint_vect, logjoint, grad_logjoint = _helper(Gen, x_tilde, current_eps,
                                                     sigma)
    ...
    for j in range(leapfrog_steps):
      ...
      if j < leapfrog_steps - 1:
        logjoint_vect, logjoint, grad_logjoint = _helper(
            Gen, x_tilde, eps, sigma)
        proposed_U = -logjoint_vect
        grad_U = -grad_logjoint
        p = p - stepsize * grad_U
    logjoint_vect, logjoint, grad_logjoint = _helper(Gen, x_tilde, eps, sigma)
    ...
    if i < burn_in and flag_adapt == 1:
      stepsize = stepsize + hmc_learning_rate * (accept.float().mean() -
                                                 hmc_opt_accept) * stepsize
    else:
